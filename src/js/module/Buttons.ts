...
export default class Buttons {
    ...
    fontInstalledMap: {[fontName: string]: boolean} = {};

    invertedKeyMap: {[command: string]: string};
    ...
    representShortcut(editorMethod: string) {
    ...
    button(options: {
        ...
        if (!this.options.tooltip && options.tooltip) {
            delete options.tooltip;
        }
        options.container = this.options.container;
        return this.ui.button(options);
    }
    ...
    isFontDeservedToAdd(name: string) {
        name = name.toLowerCase();
        return (name !== '' && this.isFontInstalled(name) && env.genericFontFamilies.indexOf(name) === -1);
    }

    colorPalette(className: string, tooltip: string, backColor: boolean, foreColor: boolean) {
            ...
            children: [
                this.button({
                    ...
                    click: (domEvent: MouseEvent) => {
                        const buttonEl = domEvent.currentTarget as Element;
                    ...
                    callback2: (buttonEls) => {
                        buttonEls.forEach((buttonEl) => {
                            ...
                            const recentColorEl = buttonEl.querySelector('.note-recent-color') as HTMLElement;
                ...
                this.ui.dropdown({
                    ...
                    callback2: (dropdownEls) => {
                        dropdownEls.forEach((dropdownEl) => {
                            if (!(dropdownEl instanceof Element)) {
                                return;
                            }

                            [].slice.call(dropdownEl.querySelectorAll('.note-holder')).forEach((item: Element) => {
                            ...
                            [].slice.call(dropdownEl.querySelectorAll('.note-holder-custom')).forEach((item: Element) => {
                                item.appendChild(this.ui.palette({
                                    colors: customColors,
                            ...
                            [].slice.call(dropdownEl.querySelectorAll('input[type=color]')).forEach((item: Element) => {
                                item.addEventListener('change', function() {
                                    const chipEl = dropdownEl
                                        .querySelector('#' + this.getAttribute('data-event'))
                                        .querySelector('.note-color-btn') as HTMLElement;
                    ...
                    click: (domEvent) => {
                        ...
                        if (eventName === 'openPalette') {
                            const pickerEl = parentEl.querySelector('#' + value) as HTMLInputElement;
                            const paletteEl = parentEl
                                .querySelector('#' + pickerEl.getAttribute('data-event'))
                                .querySelector('.note-color-row');

                            // Shift palette chips
                            const chipEls = [].slice.call(paletteEl.querySelectorAll('.note-color-btn')) as HTMLElement[];
                            ...
                            if (lists.contains(['backColor', 'foreColor'], eventName)) {
                                ...
                                const colorEl = buttonEl.closest('.note-color').querySelector('.note-recent-color') as HTMLElement;
                                const currentButtonEl = buttonEl.closest('.note-color').querySelector('.tea-editor__button--current-color') as HTMLElement;
    ...
    addToolbarButtons() {
                ...
                this.ui.dropdown({
                    ...
                    template: (item) => {
                        ...
                        if (typeof item === 'string') {
                            item = {
                                tag: item,
                                title: (Object.prototype.hasOwnProperty.call(this.lang.style, item) ? this.lang.style[item as keyof typeof this.lang.style] : item),
                            };
                        }
        ...
        for (let styleIdx = 0, styleLen = this.options.styleTags.length; styleIdx < styleLen; styleIdx++) {
            ...
            this.context.memo('button.style.' + item, () => {
                return this.button({
                    ...
                    tooltip: this.lang.style[item as keyof typeof this.lang.style],
                    click: this.context.createInvokeHandler('editor.formatBlock'),
                }).render2();
            });
        }
        ...
        this.context.memo('button.fontname', () => {
            ...
            if (this.options.addDefaultFonts) {
                ...
                styleInfo['font-family'].split(',').forEach((fontname: string) => {
            ...
            return this.ui.buttonGroup([
                ...
                callback2: (nodeEls) => {
                    nodeEls.forEach((nodeEl) => {
                        ...
                        const catcherEl = nodeEl.querySelector('.note-dimension-picker-mousecatcher') as HTMLElement;
    ...
    build(containerEl: Node, groups: string[][]) {
    ...
    updateCurrentStyle(containerEl: HTMLElement) {
        ...
        if (styleInfo['font-family']) {
            const fontNames: string[] = styleInfo['font-family'].split(',').map((name: string) => {
            ...
            [].slice.call(cont.querySelectorAll('.dropdown-fontname a')).forEach((item: Element) => {
        ...
        if (styleInfo['font-size']) {
            ...
            [].slice.call(cont.querySelectorAll('.dropdown-fontsize a')).forEach((item: Element) => {
            ...
            [].slice.call(cont.querySelectorAll('.dropdown-fontsizeunit a')).forEach((item: Element) => {
        ...
        if (styleInfo['line-height']) {
            ...
            [].slice.call(cont.querySelectorAll('.dropdown-line-height a')).forEach((item: Element) => {
                // always compare with string to avoid creating another func.
    ...
    updateBtnStates(containerEl: HTMLElement, infos: {[info: string]: () => boolean}) {
    ...
    tableMoveHandler(domEvent: MouseEvent) {
        if (!(domEvent.target instanceof Node)) {
            return;
        }

        const PX_PER_EM = 18;
        const pickerEl = domEvent.target.parentNode as Element; // target is mousecatcher
        ...
        const highlightedEl = pickerEl.querySelector('.note-dimension-picker-highlighted') as HTMLElement;
        const unhighlightedEl = pickerEl.querySelector('.note-dimension-picker-unhighlighted') as HTMLElement;
