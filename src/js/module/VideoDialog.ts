...
export default class VideoDialog {
    ...
    createVideoNode(url: string) {
        ...
        if (ytMatch && ytMatch[1].length === 11) {
            ...
            let begin = 0;
            if (peerTubeMatch[2] !== 'undefined') begin = peerTubeMatch[2] as unknown as number;
            let end = 0;
            if (peerTubeMatch[3] !== 'undefined') end = peerTubeMatch[3] as unknown as number;
            let loop = 0;
            if (peerTubeMatch[4] !== 'undefined') loop = peerTubeMatch[4] as unknown as number;
            let autoplay = 0;
            if (peerTubeMatch[5] !== 'undefined') autoplay = peerTubeMatch[5] as unknown as number;
            let muted = 0;
            if (peerTubeMatch[6] !== 'undefined') muted = peerTubeMatch[6] as unknown as number;
     ...
     * Show video dialog.
     */
    showVideoDialog(/*text: string*/): Promise<string> {
        return new Promise((resolve) => {
            const videoUrlEl = this.dialogEl.querySelector('.note-video-url') as HTMLInputElement;
            const videoBtnEl = this.dialogEl.querySelector('.note-video-btn') as HTMLElement;
